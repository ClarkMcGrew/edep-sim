...
void CaptImmersedBuilder::Init(void) {
    ...
    drift.SetApothem(1037.5*CLHEP::mm);
    drift.SetDriftLength(1000*CLHEP::mm);
...
G4LogicalVolume *CaptImmersedBuilder::GetPiece(void) {
    ...
    EDepSim::SDFactory wallFactory;
    G4VSensitiveDetector* sensitiveWall = wallFactory.MakeSD("cage","surface");
